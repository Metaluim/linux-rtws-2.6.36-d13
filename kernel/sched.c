...
static inline int rtws_policy(int policy)
{
	if (unlikely(policy == SCHED_RTWS))
		return 1;
	return 0;
}
...
static inline int task_has_rtws_policy(struct task_struct *p)
{
	return rtws_policy(p->policy);
}
...
static void init_dl_bw(struct dl_bw *dl_b)
{
	...
	raw_spin_lock_init(&def_dl_bandwidth.dl_runtime_lock);
...
struct dl_rq {
	...
	struct rq *rq;
#endif
	unsigned long tot_pushes;
	unsigned long tot_enqueues;
	unsigned long tot_picks;
	unsigned long tot_pulls;
	unsigned long tot_cs;
};

/* Work-stealing class' related fields in a runqueue */
struct global_rq {
	raw_spinlock_t lock;
	struct rb_root tasks;
	struct rb_node *leftmost_task;
	unsigned long nr_running;
	int random;
};

struct global_rq global;

struct rtws_rq {
	struct global_rq *global;
	struct rb_root pjobs;
	struct rb_node *leftmost_pjob;
	struct rb_root stealable_pjobs;
	struct rb_node *leftmost_stealable_pjob;
	unsigned long nr_running;
	u64 exec_clock;
	u64 earliest_dl;
...
struct root_domain {
	...
	/*
	 * We keep track of each CPU current
	 * and earliest stealable tasks deadline.
	 */
	struct cpudl rtwsc_cpudl; 
	struct cpudl rtwss_cpudl;
...
{
	if (task_has_rtws_policy(p) || task_has_dl_policy(p) ||
...
static inline int normal_prio(struct task_struct *p)
{
	...
	if (task_has_rtws_policy(p))
		prio = MAX_RTWS_PRIO-1;
...
/* DOUBT: do groups alter my pjobs scheduling methodology? */
	...
	RB_CLEAR_NODE(&p->rtws.pjob_node);
	RB_CLEAR_NODE(&p->rtws.stealable_pjob_node);
...
void sched_fork(struct task_struct *p, int clone_flags)
{
	...
	if (unlikely(p->sched_reset_on_fork)) {
		if (p->policy == SCHED_DEADLINE || p->policy == SCHED_RTWS ||
	...
	__releases(rq->lock)
{
	...
	if (unlikely(prev_state == TASK_DEAD)) {
		...
		/* For -rtws tasks, same as above */
		if (unlikely(task_has_rtws_policy(prev) &&
		    prev->sched_class != &rtws_sched_class))
			rtws_sched_class.task_dead(prev);
...
__setparam_dl(struct task_struct *p, const struct sched_param_ex *param_ex)
{
	...
	dl_se->stats.dmiss_max = 0;
	dl_se->stats.tot_rtime = 0;
	if (current == p)
		dl_se->parent = NULL;
	else
		dl_se->parent = &current->dl;
}
...
/*
 * This function initializes the sched_rtws_entity of a newly becoming
 * SCHED_RTWS task.
 *
 ...
 */
static void
__setparam_rtws(struct task_struct *p, const struct sched_param_ex *param_ex)
{
	...
	if (timespec_to_ns(&param_ex->sched_period) != 0)
		rtws_se->rtws_period = timespec_to_ns(&param_ex->sched_period);
	else
		rtws_se->rtws_period = rtws_se->rtws_deadline;

	rtws_se->flags = param_ex->sched_flags;
	...
	rtws_se->stats.dmiss_max = 0;
	rtws_se->stats.tot_runtime = 0;
}

static void
__getparam_rtws(struct task_struct *p, struct sched_param_ex *param_ex)
{
	struct sched_rtws_entity *rtws_se = &p->rtws;

	param_ex->sched_priority = p->rt_priority;

	param_ex->sched_deadline = ns_to_timespec(rtws_se->rtws_deadline);
	param_ex->sched_period = ns_to_timespec(rtws_se->rtws_period);
	param_ex->sched_flags = rtws_se->flags;
}

static bool
__checkparam_rtws(const struct sched_param_ex *prm, bool kthread)
{
	return __checkparam_dl(prm, kthread);
}
...
recheck:
	...
	if (policy < 0) {
		...
		if (policy != SCHED_DEADLINE && policy != SCHED_RTWS &&
	...
	/*
	 * Valid priorities for SCHED_DEADLINE are
	 * MAX_WS_PRIO..MAX_DL_PRIO-1, valid priority for SCHED_RTWS
	 * is MAX_WS_PRIO-1.
	 *
	 * SCHED_DEADLINE and SCHED_RTWS set param->sched_priority = 0 and manipulate
	 * param_ex for their own definitions. Final prio value is set on
	 * __setscheduler function, thus prio passed from user space is
	 * irrelevant (param_ex->sched_priority).
	 *
	 * The struct param_ex is null for all policies but SCHED_DEADLINE
	 * and SCHED_RTWS.
	 */ 
	if ((dl_policy(policy) && !__checkparam_dl(param_ex, !p->mm)) ||
	    (rtws_policy(policy) && !__checkparam_rtws(param_ex, !p->mm)) ||
	...
	if (user && !capable(CAP_SYS_NICE)) {
		if (rtws_policy(policy)) {
			...
			if (dline < p->rtws.rtws_deadline && dline < rlim_dline)
				return -EPERM;
		
			/* only "root" tasks with no pjobs can change the deadline */
			if (dline != p->rtws.rtws_deadline && (p->rtws.parent || p->rtws.nr_pjobs > 0))
				return -EPERM;
		}
			 ...
			 struct sched_param_ex __user *param_ex)
{
	...
	p = find_process_by_pid(pid);
	if (p != NULL) {
		if (dl_policy(policy) || rtws_policy(policy))
...
/**
 * sys_sched_delay_until_rtws - sleep according to periodic rules.
 */
SYSCALL_DEFINE1(sched_delay_until_rtws,
	const struct timespec __user *, release)
{
	return 0;
}

/**
 * sys_sched_setpolicies_rtws - set/change SCHED_RTWS sub policy
 * @pid: the pid in question.
 * @helpfirst: enqueing policy, 0 (default) - work first, 1 - help first.
 * @deterministic: stealing policy, 0 (default) - deterministic, 1 - randomized.
 */
SYSCALL_DEFINE3(sched_setpolicies_rtws,
	pid_t, pid,
	int, helpfirst,
	int, randomized)
{
	...
	struct rq *rq;
	struct global_rq *global_rq;
	unsigned long flags;

	if (helpfirst != 0 && helpfirst != 1)
		return -EINVAL;
	...
	if (p) {
		...
		if (rtws_se) {
			rq = task_rq_lock(p, &flags);
			global_rq = rq->rtws.global;

			if (helpfirst)
				rtws_se->help_first = 1;
			else
				rtws_se->help_first = 0;
...
SYSCALL_DEFINE0(sched_flush_stats_rtws)
{
	int i;
	struct rq *rq;
	struct global_rq *global_rq;
	...
	for_each_online_cpu(i) {
		...
		if (global_rq->random)
			trace_sched_rq_stat_rtws_random(rq->clock,
						 rq->rtws.exec_clock, rq->rtws.tot_steals,
						 rq->rtws.tot_pulls, rq->rtws.tot_cs);
		else
			trace_sched_rq_stat_rtws_earliest(rq->clock,
						 rq->rtws.exec_clock, rq->rtws.tot_steals,
						 rq->rtws.tot_pulls, rq->rtws.tot_cs);

		rq->rtws.tot_steals = 0;
		rq->rtws.tot_enqueues = 0;
		rq->rtws.tot_picks = 0;
		rq->rtws.tot_pulls = 0;
		rq->rtws.tot_cs = 0;
		rq->rtws.exec_clock = 0;
...
SYSCALL_DEFINE0(sched_flush_stats_dl)
{
	int i;
	struct rq *rq;
	unsigned long flags;
	...
	for_each_online_cpu(i) {
		...
		trace_sched_rq_stat_dl(rq->clock,
					rq->dl.exec_clock, rq->dl.tot_pushes,
					rq->dl.tot_pulls, rq->dl.tot_cs);

		rq->dl.tot_pushes = 0;
		rq->dl.tot_enqueues = 0;
		rq->dl.tot_picks = 0;
		rq->dl.tot_pulls = 0;
		rq->dl.tot_cs = 0;
		rq->dl.exec_clock = 0;
	...
	struct timespec __user *, rmtp)
{
	...
	/* DOUBT: lrm takes no value for -rtws tasks. will we get an error when trying to copy it to user space? */
...
void sched_idle_next(void)
{
	...
	__setscheduler(rq, p, SCHED_FIFO, MAX_RT_PRIO-1);//added
...
static void free_rootdomain(struct root_domain *rd)
{
	...
	cpudl_cleanup(&rd->rtwsc_cpudl);
	cpudl_cleanup(&rd->rtwss_cpudl);
...
static int init_rootdomain(struct root_domain *rd)
{
	...
	if (cpudl_init(&rd->rtwsc_cpudl) != 0 || cpudl_init(&rd->rtwss_cpudl) != 0)
		goto free_rto_mask;
...
void __init sched_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		init_rt_rq(&rq->rt, rq);
		init_dl_rq(&rq->dl, rq);
		init_rtws_rq(&rq->rtws, rq, &global);
...
void normalize_rt_tasks(void)
{
	...
	do_each_thread(g, p) {
		...
		if (!__rtws_task(p) && !__dl_task(p) && !rt_task(p)) {
			/*
