				       ...
				       struct sched_dl_entity *pi_se)
{
	...
	//printk(KERN_INFO "****update task, task %d dl %Ld clock %Ld****\n", p->pid, dl_se->deadline, rq->clock);
				...
				struct sched_dl_entity *pi_se)
{
	...
	struct rq *rq = rq_of_dl_rq(dl_rq);
	int reset = 0;
	//printk(KERN_INFO "replenish\n");
	dl_se->stats.dmiss_max = 1;
	/*
			     ...
			     struct sched_dl_entity *pi_se)
{
	...
	if (dl_time_before(dl_se->deadline, rq->clock)) {// ||
		...
		dl_se->stats.dmiss_max = 1;
		//dl_se->stats.dmiss_max = rq->clock - dl_se->deadline;
		dl_se->deadline = rq->clock + pi_se->dl_deadline;
		dl_se->runtime = pi_se->dl_runtime;
		overflow = 1;
	}
			     ...
			     struct timespec *rmtp)
{
	...
	//printk(KERN_INFO "********prepare new instance*******\n");
...
{
	//printk(KERN_INFO "********put prev task, task %d on cpu %d*******\n",p->pid, rq->cpu);
...
{
	struct rq *rq = task_rq(p);
	struct dl_bw *dl_b = &rq->rd->dl_bw;
	//printk(KERN_INFO "********task dead, task %d on cpu %d*******\n", p->pid, rq->cpu);
	...
	if (!p->dl.parent)
		if (p->dl.stats.dmiss_max > 0)
			trace_sched_task_stat_dl(p, rq->clock, 0);
}
