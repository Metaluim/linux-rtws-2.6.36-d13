...
void print_rtws_rq(struct seq_file *m, int cpu, struct rtws_rq *rtws_rq)
{
	int stealable_pjobs = 0;
	struct rq *rq = cpu_rq(cpu);
	unsigned long flags;

#define PG(x) \
	SEQ_printf(m, "  .%-30s: %Ld\n", #x, (long long)(rtws_rq->global->x))
#define P(x) \
	SEQ_printf(m, "  .%-30s: %Ld\n", #x, (long long)(rtws_rq->x))
#define __P(x) \
	SEQ_printf(m, "  .%-30s: %Ld\n", #x, (long long)(x))
#define PN(x) \
	SEQ_printf(m, "  .%-30s: %Ld.%06ld\n", #x, SPLIT_NS(rtws_rq->x))
	...
	if (rtws_rq->leftmost_stealable_pjob)
		stealable_pjobs = 1;
