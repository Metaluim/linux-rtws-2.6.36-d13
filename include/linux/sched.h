...
struct sched_rtws_entity {
	...
	u64 rtws_deadline; //relative deadline
	u64 rtws_period;
	unsigned int flags;	/* specifying the scheduler behaviour   */
	int help_first, throttled, stolen;
	struct rtws_job job;
	unsigned long nr_pjobs;
	struct rb_node pjob_node;
	struct rb_node stealable_pjob_node;
	struct sched_rtws_entity *parent;
	struct sched_stats_rtws stats;
};
 ...
 *
 * SCHED_DEADLINE tasks have negative priority equal to
 * MAX_DL_PRIO-1, reflecting the fact that any of them
 * hashigher prio than RT and NORMAL/BATCH tasks.
 */
...
/* SCHED_RTWS tasks have negative priority equal to MAX_RTWS_PRIO-1, since 
 * this priority has no impact on our scheduling strategy (only job's 
 * absolute deadline matters), but still reflecting
 * the fact that any of them has higher priority than SCHED_DEADLINE tasks
 */

#define MAX_RTWS_PRIO		(MAX_DL_PRIO - 1)

static inline int rtws_prio(int prio)
{
	if (unlikely(prio == MAX_RTWS_PRIO - 1))
		return 1;
	return 0;
}

static inline int rtws_task(struct task_struct *p)
{
	return rtws_prio(p->prio);
}

/*
 * We might have temporarily dropped SCHED_RTWS policy,
 * but still be a -rtws task!
 */
static inline int __rtws_task(struct task_struct *p)
{
	return rtws_task(p) || p->policy == SCHED_RTWS;
}

static inline int dl_prio(int prio)
{
	if (unlikely(prio < MAX_DL_PRIO && prio >= MAX_RTWS_PRIO))
		return 1;
	return 0;
}
